...
| --- | --- | --- | --- |
| **1** *(complete — 1a, 1b, 1c ✅)* | [`complete/phase-1-tile-registry.md`](complete/phase-1-tile-registry.md) | Build `TileRegistry`; render + gen read tiles by id. Sliced sheets fully data-driven (`NatureTile`/`UrbanTile3` deleted); all four grid sheets flipped to id-addressed autotile/variant blocks; per-cell labels folded in, test-only pickers retired. | Kills "enum order = PNG order" + hardcoded `(col,row)` fragility. **This is the "tiles within a tileset" JSON.** |
| **2** *(complete)* | [`complete/phase-2-doodad-pools.md`](complete/phase-2-doodad-pools.md) | `GenMappingRegistry` + `*.mapping.json` shipped, carrying all three sections: ✅ `doodadPools` + cover (`DoodadDef`); ✅ `groundRender` (GroundKind→block + generic `GroundRenderSystem.drawGroundBlock`); ✅ `fillers` params (`FillerParams` — `NatureZoneFiller` pools/chances). Optional later: a `"filler"` dispatch field; other fillers' scatter tunables. | Tile recipes are data; re-tuning densities/skins is a JSON edit. **This is the "mapping to generated things" JSON.** |
