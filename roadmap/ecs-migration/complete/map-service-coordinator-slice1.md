# Story: MapEditor coordinator — Slice 1 (runtime map-modification cycle)

**Shipped `c49eea7`** (2026-05-28). First slice of the
[`map-service-coordinator`](map-service-coordinator.md) story:
a new `MapEditor` (in `battle.world`) now owns the cross-domain runtime
...
> **Naming note (2026-06-30):** this class shipped as `MapService` (the name in
> the `c49eea7` commit). It was renamed to **`MapEditor`** in the Service/System
> naming sweep — it owns no component state (a stateless cross-domain mutation
> coordinator), so it isn't a data-owner `*Service`. References below use the
> current name `MapEditor`.

## What landed vs. planned

- **As planned:** `damageWall` / `peelRoofAround` / `destroyRoof` /
  `flipCellToRubble` moved off `NavigationService` onto `MapEditor`,
...
- **Constructor shape:** `MapEditor(NavigationService)`. It aliases
...
- **Consumers repointed (3):** `Detonations`, `TurretDemolitionSystem`,
  ...
  field for a `MapEditor mapEditor` field — clean swaps because each
  used `navigation` **only** for the moved ops (verified by grep before
  touching them).
- **Sim wiring:** `BattleSimulation` builds `mapEditor` right after
  `navigation`; `damageCell` stays a thin delegate (now →
  `mapEditor.damageWall`); the roof-collapse decal sink wiring moved from
  `navigation.setRoofCollapseSink` → `mapEditor.setRoofCollapseSink`.
...
- **`NavigationService.markZoneGraphDirty()` stays public** — it's now
  called by MapEditor rather than internally; the zone-graph dirty flag +
  `flushZoneGraphIfDirty` drain remain navigation's (they're navigation
  state).

## Package home

`MapEditor` lives at the **top level** of `battle.world` (the map feature
...
the top level is no longer a "pure container" — it hosts the MapEditor
...
- **Slice 2 — generation cycle** (stretch): folding the
  ...
  MapEditor. Larger surface, lower smell; scope as a follow-on only if the
