# 2026-05-21 — Campaign-tier contracts loop, end-to-end

Implementation half of the contract design that landed earlier today
(see [`2026-05-21.md`](2026-05-21.md) for the skeleton + architecture
commitments, and [`../campaign/contracts.md`](../campaign/contracts.md)
for the design). This session takes the contract layer from "design
doc + empty SoA table slot" to **a playable loop**: T1 patrons spawn
offers, patrons show up as clients on local planets, the player picks
a mission with a salvage-vs-cash negotiation knob, the battle resolves,
and the resolver bridge writes the outcome back to `CampaignState` with
rep + phase deltas.
...
1. **Loot picker** (`loot.md` design + implementation) — biggest user
   value. Three-layer salvage model is already plumbed; what's missing
   is the item pool generator (weapons / supplies / fuel / marines /
   AI cores per the doc), the roll weighted by entitlement + enemy
   faction + planet industries, the post-battle picker grid with
   cargo-capacity check, and the 75% fence-on-spot for overflow.
   MechWarrior Mercenaries vibe. Probably its own design doc first.
2. **Offer expiry + patron archetypes** — small polish round. Offers
   lapse after N days; archetype byte populated at seed time and
   surfaced in briefing flavor text (the patron-narrative-discoverable
   feedback memory is the design constraint).
3. **ContractGenerator for GARRISON + CADRE** — stationing contracts
   with retainer payment. New mechanic since they pay over time, not
   on completion. Closer to the contract design's "two-mode dichotomy"
   from `contracts.md`.
4. **Briefing OFFERED → ACTIVE transition** — only matters when
   multi-phase contracts (PLANETARY_ASSAULT) exist. Defer.
