 ...
 * dispatches to a behavior based on {@link Unit#role} (with fall-back as a
 * pre-dispatch override). Each behavior is stateless across ticks — all
 * per-unit state lives on {@link Unit} itself — so a single instance can
...
public interface UnitBehavior {
    void update(Unit u, BattleSimulation sim);
}
