 ...
 *
 * <p>Carries only what the dead-sprite render needs today: the {@link UnitType}
 * (for the corpse sheet + render scale) and the {@code deathPoseIdx} (which
 * frozen prone-pose frame to draw). Position is <b>not</b> duplicated here — it
 * lives in the shared render-position component keyed by the same entity id, so
 * the corpse composes its location rather than copying it. Faction / cell can be
 * added when a consumer (medic targeting, a diagnostics dump) needs them; kept
 * minimal until then.
 */
public final class DeadBody {
    ...
    public DeadBody(UnitType type, int deathPoseIdx) {
        this.type = type;
