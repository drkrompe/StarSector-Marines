...
/**
 * Infantry slice of the combatant loop: acquire target, fire primary/secondary
 * when in range with LOS, otherwise path to a firing position. Sibling of
 * {@link MechCombatantBehavior}; {@link CombatantBehavior} picks between the
 * two based on {@link Unit#mech}.
 *
 * <p>Squad cohesion lives here: if a marine has drifted more than
 * {@link #COHESION_RADIUS} cells from the rest of the squad, they path toward
 * the centroid before resuming normal engagement, so fireteams move as a group
 * instead of scattering. Cohesion is a soft leash — once within radius, normal
 * targeting takes over. A marine with a great firing position 10 cells out
 * from the squad will be pulled back; one with a position 5 cells out will hold.
 */
public final class InfantryCombatantBehavior implements UnitBehavior {

    public static final InfantryCombatantBehavior INSTANCE = new InfantryCombatantBehavior();
    ...
    private InfantryCombatantBehavior() {}
