...
public final class HybridAStarPlanner {
     ...
     * grid-distance heuristic and Reeds-Shepp analytic expansion as
     * {@link #refine}. Two differences make it cheap and high-success where the
     ...
     * goal pose) when the search closed via analytic expansion. Unlike
     * {@link #extractPath} there is no coarse-guide prefix to prepend — the
     * local search starts from the vehicle's live pose. When the search closed
    ...
    private static float[] computeGridDistance(int goalX, int goalY,
        ...
        return computeGridDistance(goalX, goalY, grid, w, h, 0, 0, w - 1, h - 1);
    }
